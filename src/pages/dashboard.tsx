...
import { SkillListPage as ShowCaseList } from 'pages/skills';

function DashboardPage() {
  const { session } = useSession();

  const skillStat = useSWR<PaginatedInterface<SkillInterface>>(
    () => `/skills`,
    () => getSkills({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Skills',
      result: skillStat?.data?.totalCount,
      isLoading: skillStat.isLoading,
    },
  ];
