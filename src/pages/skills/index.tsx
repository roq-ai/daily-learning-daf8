...
type ColumnType = ColumnDef<SkillInterface, unknown>;
...
export function SkillListPage(props: SkillListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<SkillInterface>>(
    () => `/skills?params=${JSON.stringify(params)}`,
    fetcher,
  );
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/skills/view/${record.id}`} passHref legacyBehavior>
                <Button
